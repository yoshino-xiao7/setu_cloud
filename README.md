...
**亦可 YK（原雪涼云）** 的 Web 前端，提供图片浏览、收藏分享、AI 绘画、在线音乐播放器、账户与 API Key 管理，以及管理员后台。采用 Aurora Glassmorphism（极光毛玻璃）视觉风格，支持桌面和移动浏览器。

品牌英文标识为 **YIKE**，简称 **YK**；仓库目录仍为 `setu_cloud`。访问 [亦可 YK](https://cloud.yukiryou.icu/)，或配合 `setu_api_full` 后端进行本地开发。
...
- **Session 登录** — HttpOnly `SID` Cookie 保存会话，配合 `signSecret` 进行受保护请求的 HMAC 签名
...
- **关于亦可** — 品牌沿革、产品介绍、看板娘、图库收录统计与快捷入口
- **隐私政策** — 合规隐私协议页
