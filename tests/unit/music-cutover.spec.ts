...
import { pinCanonicalHistory, usesCanonicalHistory } from '@/api/musicCohort'
import { musicFlags } from '@/api/musicFlags'
import { admitPlaybackSession, musicV2Api, resolveSongLyrics, resolveSongPlayback } from '@/api/musicV2'
...
beforeEach(() => {
  vi.resetAllMocks()
  vi.mocked(http.delete).mockResolvedValue({ status: 200, data: '' })
  state.auth.user = { id: state.auth.user!.id + 1 }
  mutable.usesV2History = false
  mutable.usesV2Playback = false
  mutable.usesV2Lyrics = false
})
describe('cutover routing and safe rollback', () => {
  ...
  it('bounded release headers reject unsafe and oversized values', () => {
    ...
    for (const build of ['x\r\nCookie: secret', 'a'.repeat(41), 'user@example.com', ''])
      expect(musicClientHeader('2.6.0', build)).toBeUndefined()
  })
  it('false routes history, lyrics and retained hot/daily to legacy', async () => {
    ...
    await musicHistoryApi.getHistory()
    await musicHistoryApi.getCount()
    await musicHistoryApi.clearHistory()
    await resolveSongLyrics('123')
    await userMusicApi.getRecommendSongs()
