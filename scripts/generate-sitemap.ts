...
async function fetchDynamicPages(): Promise<DynamicPage[]> {
  ...
  try {
    ...
    const json = await res.json() as { data?: { list?: Record<string, unknown>[], items?: Record<string, unknown>[], records?: Record<string, unknown>[] } }
    const data = json.data || json
    const list: Record<string, unknown>[] = (data as Record<string, unknown[]>).list || (data as Record<string, unknown[]>).items || (data as Record<string, unknown[]>).records || []
    ...
    for (const item of list) {
      ...
      // 收集唯一用户 ID
      if (item.userId) {
        userIds.add(item.userId as number)
      }
    }
    ...
    console.warn(`⚠️  Could not fetch dynamic pages: ${msg}`)
  }
