...
export function usesCanonicalHistory(): boolean {
  ...
  try { return musicFlags.usesV2History || volatilePins.has(owner) || localStorage.getItem(owner) === 'true' }
  catch { return musicFlags.usesV2History || volatilePins.has(owner) }
}
export function pinCanonicalHistory(): void {
  ...
  if (localStorage.getItem(owner) !== 'true') throw new Error('无法保存播放会话，请检查浏览器存储权限')
