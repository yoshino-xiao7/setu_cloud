...
varying vec2 vUv; uniform sampler2D tArt; uniform vec2 uView; uniform float uFoil,uTime;
...
void main(){
 vec2 uv=vUv; vec3 art=texture2D(tArt,uv).rgb;
 ...
 vec3 color=mix(art,overlay(art,foil),strength*.36);
 float sweep=pow(max(0.,sin((uv.x*.83+uv.y*.35+uView.x*1.8+uView.y*.9)*6.283)),14.);
 color+=foil*sweep*strength*.25;
 ...
 color+=vec3(.85,.91,1.)*cross*step(.94,seed)*twinkle*strength*.12;
 float border=1.-smoothstep(.006,.012,min(min(uv.x,1.-uv.x),min(uv.y,1.-uv.y)));
 color=mix(color,mix(vec3(.82,.78,.67),foil,.7),border*.75);
 gl_FragColor=vec4(color,1.);
}`

/** Owns one card's GPU resources; the caller owns animation and visibility. */
export function createMascotFoil(canvas: HTMLCanvasElement, image: HTMLImageElement) {
