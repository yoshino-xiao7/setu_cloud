...
const { formatDate, goToApiKeys, handlePageChange, handlePageSizeChange, keyError, keyState, keyUsagePercent, logsError, overview, overviewError, pagination, refreshLogs, tableState, fetchOverview, fetchKeyStats } = useUserDashboard()
</script>
  ...
  <UiBoard class="dashboard-page ui-page">
    ...
    <section class="board__section">
      ...
      <UiRecordBoard :items="tableState.data" :loading="tableState.loading" :error="logsError" empty="暂无调用日志">
        <template #error>
          {{ logsError }}<NButton text type="primary" @click="refreshLogs">
            重试
          </NButton>
        </template>
