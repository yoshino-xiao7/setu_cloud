...
const { data, loading, error, reload } = useMusicResource(true, async () => {
  ...
  if (!Array.isArray(result?.dailySongs)) throw new Error('每日推荐暂不可用')
...
<template>
  <section class="legacy-daily">
    ...
    <div v-else-if="error" role="alert">{{ error }} <NButton @click="reload">重试</NButton></div>
    <NEmpty v-else-if="!data?.length" description="暂无每日推荐" />
    <div v-for="song in data ?? []" :key="song.id" class="daily-row">
      <span>{{ song.name }}</span><NButton :aria-label="`播放 ${song.name}`" @click="player.playSong(song)">播放</NButton>
    </div>
  </section>
</template>
<style scoped>
