...
const p17Tracks = Array.from({ length: 24 }, (_, index) => ({
  id: `netease:track:${9007199254740993n + BigInt(index)}`,
  source: 'netease',
  title: `P17 Song ${index + 1}`,
  artists: [{ id: 'netease:artist:1001', name: 'P17 Artist', artwork: null }],
  album: null,
  durationMs: 180000,
  artwork: null,
  mvId: index === 0 ? 'netease:mv:700000' : null,
  aliases: [],
  translatedTitle: null,
  availability: { status: 'playable', reason: null, maxQuality: 'lossless' },
}))
...
function p17Page<T>(items: T[], config: InternalAxiosRequestConfig) {
  const offset = Number(config.params?.offset ?? 0)
...
function p17Handler(key: string): MockHandler | undefined {
  if (key === 'POST /user/music/rollout/events')
    return () => mockResponse(204, '')
  if (key === 'GET /user/music/rollout/capabilities')
    return () => ({ version: 1, admitNewPlaybackSession: window.sessionStorage?.getItem('cutover:admission') !== 'closed', validForSeconds: 30 })
  if (key === 'GET /user/music/recommend/songs')
    return () => ({ data: { dailySongs: p17Tracks.slice(0, 4).map(track => ({ id: track.id.replace('netease:track:', ''), name: track.title, artists: [{ id: '1', name: 'P17 Artist' }], album: { id: '1', name: 'Daily' }, duration: 180000 })) } })
  if (key === 'GET /user/music/v2/library/history')
    return config => p17Page(cutoverHistoryCleared ? [] : p17Tracks.map(track => ({ ownerId: 'setu:user:1', trackId: track.id, lastPlayedAt: '2026-09-06T00:00:00Z', track })), config)
  if (key === 'DELETE /user/music/v2/library/history') {
    return () => {
      cutoverHistoryCleared = true
      return mockResponse(204, '')
    }
  }
  if (key === 'POST /user/music/v2/library/history') {
    return () => {
      cutoverHistoryCleared = false
      return mockResponse(204, '')
    }
  }

  if (key === 'GET /user/music/history/count')
    return () => p17Tracks.length
  if (key === 'GET /user/music/history') {
    return (config) => {
  ...
  if (key === 'GET /user/music/v2/home') {
    return () => ({ generatedAt: '2026-09-06T00:00:00Z', sections: [
  ...
  if (key === 'GET /user/music/v2/rankings')
    return () => ({ items: [{ ...p17Provider, isRanking: true, title: 'P17 热歌榜' }], source: p17Source('ranking') })
  if (key === 'GET /user/music/v2/search')
    return config => ({ query: config.params?.keywords, scope: 'tracks', best: null, sections: [{ scope: 'tracks', status: 'loaded', error: null, items: p17Page(p17Tracks, config) }] })
  ...
  if (playlist) {
    return (config) => {
      ...
      if (entity.id !== id)
        return mockResponse(404, { message: '歌单不存在' })
  ...
  if (remove) {
    return () => {
      ...
      if (!p17Tracks.some((_, index) => String(9007199254750001n + BigInt(index)) === remove[1]))
        return mockResponse(404, { message: '成员关系不存在' })
  ...
  if (track) {
    return (config) => {
      ...
      if (track[2] === 'lyrics')
        return { trackId, kind: 'line', hasTranslation: true, contributors: [], lines: [{ text: 'A quiet melody', translation: '一段安静的旋律', startMs: 0, durationMs: 60000, words: [] }] }
      if (config.headers.get('X-Setu-Playback-Contract') !== '3.0.0')
        return mockResponse(400, { message: '需要 Playback Contract 3.0.0' })
...
export function createMockAdapter(defaultAdapter: AxiosAdapter): AxiosAdapter {
  return async (config) => {
      ...
      const empty = (value: unknown): void => {
        ...
        if (Array.isArray(record.items)) {
          ...
          if ('hasMore' in record)
            Object.assign(record, { hasMore: false, nextOffset: null, total: 0 })
        }
    ...
    const response = isMockHttpResponse(result)
      ...
      : ok(config, result)
    if (/\/user\/music\/v2\/tracks\/[^/]+\/playback$/.test(normalizePath(config)))
      response.headers['x-setu-playback-contract'] = '3.0.0'
